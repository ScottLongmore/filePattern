...
class fileAction(object):
    ...
    def findInputFiles(self,inputs=[]):
        ...
        for inputKey in inputs: 
            ...
            # Replace dir/re templates (%string) with meta section values to narrow input searches
            if self.config['meta']:
               newDirs=[]
               for dir in input['dirs']:
                   newDirs.append(repStrTmpl(dir,self.config['meta']))
                   #print "\t\tDIRS: {}".format(dir)
                   #print "\t\tNEWDIRS: {}".format(newDirs[-1])
               input['dirs']=newDirs
               #print "\t\tRE: {}".format(input['re'])
               input['re']=repStrTmpl(input['re'],self.config['meta'])
